...
async function setupSettings() {
    ...
    const setVal = (id: string, val: any, defaultVal: any) => {
        const el = document.getElementById(id) as HTMLInputElement | HTMLSelectElement;
        if (el) {
            if (el.type === 'checkbox') (el as HTMLInputElement).checked = val !== undefined ? val : defaultVal;
    ...
    inputs.forEach(id => {
        ...
        if (el) {
            el.addEventListener('change', (e) => {
                chrome.storage.sync.set({ [id]: parseFloat((e.target as HTMLInputElement).value) });
            });
        }
    });
        ...
        if (el) {
            el.addEventListener('change', (e) => {
                ...
                if (id === 'autoRefreshEnabled') {
                    ...
                    const refreshRateInput = document.getElementById('refreshRate') as HTMLInputElement;
                    const randomRefreshInput = document.getElementById('randomRefresh') as HTMLInputElement;
    ...
    if (selectAudio) {
        selectAudio.addEventListener('change', (e) => {
            chrome.storage.sync.set({ audio: (e.target as HTMLSelectElement).value });
        });
    }
    ...
    if (playAudioBtn) {
        playAudioBtn.addEventListener('click', async () => {
            const audio = (document.getElementById('selectAudio') as HTMLSelectElement).value;
            const volume = parseFloat((document.getElementById('volume') as HTMLInputElement).value) / 100;
...
async function loadStudies() {
    ...
    const render = async (list: Study[]) => {
        ...
        list.forEach(study => {
            ...
            const createDetail = (label: string, value: string) => {
...
async function loadHistory() {
    ...
    const history: StudyHistory = result.studyHistory || { studies: [], lastUpdated: new Date().toISOString() };
    ...
    const render = (list: Study[]) => {
        ...
        list.forEach(study => {
            ...
            const createDetail = (label: string, value: string, style?: string) => {
                const item = document.createElement('div');
                item.className = 'detail-item';
                const labelSpan = document.createElement('span');
                labelSpan.className = 'detail-label';
...
async function markStudyCompleted(studyId: string) {
    const result = await chrome.storage.local.get("studyHistory");
    let history: StudyHistory = result.studyHistory || { studies: [], lastUpdated: new Date().toISOString() };
