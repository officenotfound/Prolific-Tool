...
async function handleMessages(message: { target: string; type: any; data?: any; }): Promise<void> {
    ...
    switch (message.type) {
        case 'play-sound':
            // Check if data contains audio/volume overrides
            const audioData = message.data || {};
            await handlePlaySound(audioData.audio, audioData.volume);
            // Only send notification if it was a generic alert, not a test sound
            if (!message.data) {
                sendNotification();
            }
            break;
