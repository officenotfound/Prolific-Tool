...
function initDarkMode() {
    return __awaiter(this, void 0, void 0, function* () {
        const result = yield chrome.storage.sync.get("darkMode");
        if (result["darkMode"]) {
...
function observeStudyChanges() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield chrome.storage.sync.get(["refreshRate", "autoRefreshEnabled", "randomRefresh"]);
        ...
        const randomRefresh = result["randomRefresh"];

        if (autoRefreshEnabled && (refreshRate && refreshRate > 0 || randomRefresh)) {
            const setNextPoll = () => {
                let delay = 60000; // Default 60s

                if (randomRefresh) {
                    // Random between 15s and 60s
                    delay = Math.floor(Math.random() * (60000 - 15000 + 1)) + 15000;
                } else if (refreshRate && refreshRate > 0) {
                    delay = refreshRate * 1000;
                }

                globalInterval = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                    const node = yield waitForElement(targetSelector);
                    if (node && !isProcessing) {
                        yield extractAndSendStudies(node);
                    }
                    setNextPoll(); // Schedule next poll
                }), delay);
            };

            if ((refreshRate && refreshRate > 0) || randomRefresh) {
                setNextPoll();
            }
        });
}
...
function extractStudies(targetNode) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // Inject High Pay CSS
        if (!document.getElementById('prolific-notifier-styles')) {
            const style = document.createElement('style');
            style.id = 'prolific-notifier-styles';
            style.textContent = `
                .high-pay-study {
        ...
        studyElements.forEach((study) => {
            ...
            // High Pay Check (Default > £10/hr)
            if (rewardPerHour) {
                const rate = getFloatValueFromMoneyStringContent(rewardPerHour);
                if (rate >= 10) {
                    study.classList.add('high-pay-study');
                }
            }
